#### UTF8、UTF16、ASCLL、UNICODE、GB2312等是什么


1. 计算机只有二进制0和1，为了能让计算机显示各种符号，就需要一张映射表，ASCII、Unicode、GB2312就属于映射表，本质也是一种编码规则，我们通常称作字符集，当计算机识别到某个数字，通过字符集就能查找到其字符并显示到屏幕上。

2. 计算机刚问世，只有一个ASCII(0 ～ 127对应127个字符)字符集，后来随着计算机普及，ASCII无法满足多个国家的字符
显示，各个国家就出现了自己的字符集。英文是通用语言，所以ASCII的映射关系基本被所有国家沿用，可以说ASCII字符集是其他字符集的子集。GB2312就是专门为汉字设计的字符集。

3. 当互联网发展起来以后，国家与国家之间的通信变得频繁，在计算机的传输中，我往一个韩国人的电脑发送两个符号"你好"，传输过去的实际是一串二进制数字，对方的电脑上会根据他们自己的字符集重新识别，翻译后也能得到一个符号，但肯定不是汉语中的"你好"的意思，大概率是乱码。为了解决不同字符集导致的乱码问题，某组织就创造了一个涵盖世界上所有字符的庞大的字符集，这就是UNICODE。

4. UNICODE实际上就是将世界上所有国家用的字符集集合在一起，保证了符号翻译不会出现乱码。但是这样又出现了一些新的问题，比如美国，原本只需要一个字节就能表示这个国家所有的字符，而UNICODE由于过于庞大，每个字符都需要两个字节来表示，甚至目前有的字符已经超过两个字节，字符集这样设计没什么问题，但是当把字符集对应的数字读区到内存中传输或者保存到磁盘中时，则会浪费很多空间。尤其诸如美国这样原本使用ASCLL字符集的国家，这无疑浪费了他们电脑上的存储空间，为了解决这一问题，UTF-8、UTF-16、UTF-32应运而生。

    
5. UTF-8，是针对Unicode字符集的二次编码规则，实际上也是一种映射。字符到数字是第一层映射，为了解决计算机显示字符的问题，数字到数字再做一层映射，为了解决存储空间大的问题。换言之，UTF-8编码可以理解成在字符集的输出上进行的二次编码，这里的8不是一个字节的意思，而是可变字节数的意思。二次编码有一套转换算法，具体再看吧。
    
6. UTF-16、UTF-32，不是可变编码规则，分别用2、4个字节编码字符集输出的数字。
    
    
7. 假设通过UTF-8编码，UTF-8编码规则如下：
